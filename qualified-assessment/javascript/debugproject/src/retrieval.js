...
const getAuthorBodies = async (name) => {
  let authorURLs;
  if (name) {
...
const getAuthors = async (name) => {
  if (allAuthors.length == 0) {
    allAuthors = await getAuthorBodies(name);
  }
  return flatten(allAuthors.map((e) => parseAuthorInfo(e.data)));
};
